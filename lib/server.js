...
var handleRequest = function(req, res, proxy) {
  ...
  if ((req.method == 'GET' && project) && (Object.keys(versionData).indexOf(project) > -1 || version == 'latest') ) {
    console.log(
      'Serving package from GitHub ('.green +
      versionData[project]._github.green + ') as: '.green +
      req.url.magenta
    );
    if (version) {
...
server.proxy.on('end', function(req, res) {
  console.log('Serving package from "registry.npmjs.org": '.green + req.url.magenta);
});
...
var prepareVersionsForNpm = function(versionData) {
  ...
  for (projectName in versionData) {
    ...
    retData[projectName] = {
      ...
      _github: latestVersionData.__github,
