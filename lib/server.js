...
var init = function(config, callback) {
  ...
  var getInfo = function(repos, callback) {
    if(repos.length == 0) {
      versionData = {};
      callback(null, versionData, server);
    } else {
...
var setupServer = function(config, callback) {
  ...
  setInterval(function() {
    console.log('Updating GitHub information.')
    prevVersionData = versionData;
    init(config, updateCallback);
...
var printServedPackages = function(data) {
  if (Object.keys(data) == 0) {
    console.log("Serving no GitHub projects!");
  } else {
    console.log("Serving GitHub projects:");
  ...
  console.log('GitHub rate limit remaining (reset each hour): ' + github.ratelimitRemaining + ' requests');
}
...
exposed = module.exports = {
  ...
  printServedPackages: printServedPackages,
  setup: setupServer
};
